 ...
 * publish-gated browser. Seeds from what's live (`visible`), tracks the selection⇄live
...
describe('useOnSiteSelection', () => {
  it('seeds the selection from currently-live items, with no pending delta', () => {
    const items: Item[] = [{ id: 'a', visible: true }, { id: 'b', visible: false }]
  ...
  it('re-seeds to the new live set when items change (post-publish), clearing the delta', () => {
    const initial: Item[] = [{ id: 'a', visible: true }, { id: 'b', visible: false }]
    ...
    rerender({ items: [{ id: 'a', visible: true }, { id: 'b', visible: true }] })
