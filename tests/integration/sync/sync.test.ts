 ...
 * seed artists and wipe their `tracks` after every test — a teardown that destroys rows the
...
beforeAll(async () => {
  ...
  artistA = (await createThrowawayArtist(svc, 'Spotify sync A', asA)).id
  artistB = (await createThrowawayArtist(svc, 'Spotify sync B', asB)).id
})
...
describe('syncSpotifyTracks', () => {
  ...
  it("CRITICAL: cannot sync into another tenant's artist", async () => {
    ...
    const { error: plantErr } = await svc.from('tracks').insert({ artist_id: artistB, title: "B's own song", source: 'manual' })
    ...
    const { data: after } = await svc.from('tracks').select('title').eq('artist_id', artistB)
    expect(after).toHaveLength(1) // the witness, and nothing the sync tried to add
    expect(after![0].title).toBe("B's own song")
  })
})
