...
describe('every + turns blue on hover and keyboard focus (Sam, 2026-09-23, said more than once)', () => {
  ...
  it('CRITICAL: no manager-tool call site opts a + out of the accent', () => {
    // An explicit tone wins by design, so one `tone="default"` on a + would quietly undo
    // Sam's rule at that call site. RowIcon is shared by every manager tool since the
    // (manager-tools) reorg (2026-09-24), so read every RowIcon under (manager-tools)/**
    // off the source — Brand's +s and any other tool's.
    const root = join(process.cwd(), 'src/app/artists/[id]/(dashboard)/(manager-tools)')
    const files: string[] = []
