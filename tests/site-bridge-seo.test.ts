...
describe('jsonLdGraph', () => {
  ...
  it('CRITICAL: images by kind — photo → ImageObject with the alt, artwork → VisualArtwork, none/logo → left out', () => {
    expect(byType('ImageObject')).toEqual([{ '@type': 'ImageObject', contentUrl: 'https://cdn.example.com/a1/gallery/skeen-oslo.jpg', caption: 'Skeen, Oslo', creditText: 'Skeen' }])
    expect(byType('VisualArtwork')).toEqual([{ '@type': 'VisualArtwork', name: 'Blue Study by Skeen', image: 'https://cdn.example.com/a1/gallery/blue-study.jpg', creator: { '@id': `${ORIGIN}/#artist` } }])
    expect(nodes.some((n) => JSON.stringify(n).includes('ornament') || JSON.stringify(n).includes('logo.png') && n['@type'] !== 'MusicGroup')).toBe(false)
  })
  it('no mediaUrl → no image nodes at all', () => {
    const g = jsonLdGraph(payload(), { origin: ORIGIN })
