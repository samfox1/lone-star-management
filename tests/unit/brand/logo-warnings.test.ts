...
import { logoWarnings, NOT_FLAT_TEXT } from '@/app/artists/[id]/(dashboard)/brand/logos/warnings'
import type { LogoAnalysis } from '@/lib/image-checks'

vi.mock('@/app/artists/[id]/(dashboard)/brand/actions', () => ({
  deleteLogoAction: vi.fn(async () => ({})),
  setBrandAssetAction: vi.fn(async () => ({})),
}))
import { deleteLogoAction, setBrandAssetAction } from '@/app/artists/[id]/(dashboard)/brand/actions'
import { iconsFramedFrom, NO_ICONS, removeLogo, removeQuestion, type IconUse } from '@/app/artists/[id]/(dashboard)/brand/logos/remove'
