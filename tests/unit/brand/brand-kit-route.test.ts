...
import { fakeClient, isOwnershipRead, type Call, type Reply } from '@tests/unit/brand/_fake-client'
...
import { GET } from '@/app/artists/[id]/(dashboard)/brand/kit/route'
...
describe('brand kit route — what it leaves out, and says so', () => {
  ...
  it('CRITICAL: the total is capped — an object that would overflow is skipped and reported, smaller ones still fit', async () => {
    const { MAX_KIT_BYTES } = await import('@/app/artists/[id]/(dashboard)/brand/kit/kit-entries')
