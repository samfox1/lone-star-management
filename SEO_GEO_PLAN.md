...
- [x] SEO / GEO editor v3 (2026-08-28, /design-variations-html rounds 1–4, winner R4-V02 'Two-line ledger'): tools rail = the assets 76px icon rail; sections as the filter-bar pill group; 'Test with ▾' top-right; floating password-gated Publish bar; AI visibility = one centred column (max-w-4xl) of question/answer rows with edit + remove, in-place editing, centred 'Add question' (extra slots), fixed questions reset to automatic on remove. Other sections still on the card layout — restyle next.
- [x] SEO / GEO editor v2 (Sam, 2026-08-28): a second panel of sections (listing · share image · facts · about · alt tags · AI visibility · test); the bio and alt tags edit in place (thumbnail preview); Publish only while unpublished; the five frozen questions have the artist's ANSWERS (faq_answer_1..5) published to /faqsheet — sitemap-only, never linked on the site (bridge 0.33.5: probePrompts, faqEntries, faqPageJsonLd). Dashboard restyle PAUSED by Sam (list+inspector mocks rejected); only SEO/GEO proceeds.
