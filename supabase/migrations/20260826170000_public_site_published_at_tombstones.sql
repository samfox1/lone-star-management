-- `published_at` counts tombstones (SEO_GEO_PLAN review, 2026-08-26): a delete + publish
-- writes a `_deleted` revision that `published_revisions()` hides, so the old max never
-- moved and the sitemap lastmod lied after a removal. Reproduced verbatim from
-- 20260826150000 with ONE expression changed.
