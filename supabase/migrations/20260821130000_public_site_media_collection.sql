-- The media payload gains `collection` (20260821120000): which declared image pool a
-- photo fills, so a site with two collections (ftbk: desktop works vs the Photos app)
-- can tell them apart on the wire. Additive; null on revisions published before
-- `collection` joined the snapshot list, which means "the first declared collection" —
-- exactly what every one-gallery site already renders.
-- Reproduced verbatim from 20260820140000 with ONE line added.
