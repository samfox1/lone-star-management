-- get_public_site: the TRACKS branch reads on_site from the snapshot (PRESENCE_PLAN.md S1).
--
-- Companion to 20260910130000, which did the same for the three music doors and
-- backfilled on_site into each track's and release's latest revision. This function is
-- large and is re-created whole, copied from 20260826170000 with ONE gate changed — the
-- tracks branch. Tour dates, merch, links, videos and media keep the live-row gate:
-- those go straight to the site by design (tour/merch) or by ADR 0009 (the rest).
--
-- Fallback order, as everywhere: snapshot on_site → live row on_site → true.
