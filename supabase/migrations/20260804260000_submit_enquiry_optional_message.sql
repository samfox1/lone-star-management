-- submit_enquiry stops requiring a message.
--
-- The companion to 20260804250000, which relaxed the CHECK. Separate file because that
-- one is already applied, and editing an applied migration leaves the file disagreeing
-- with what actually ran — the one thing a migration history exists to prevent. (Same
-- mistake I made earlier today with 20260804190000; noting it so the next reader sees the
-- split was deliberate.)
--
-- A demo is a link and some audio. Requiring a covering sentence rejected every one of
-- skeen's demo submissions with missing_field. The real rule — an enquiry must carry
-- SOMETHING — is enforced in the Edge Function (`hasContent`), because neither the demo
-- link nor the attachments exist at this point in the transaction.
--
-- Length is still checked HERE, because it is knowable and because violating it would
-- RAISE, which rolls back the contact_attempts row and blinds the rate limiter.
