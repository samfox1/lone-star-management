-- `published_at` joins the public payload (SEO_GEO_PLAN B1): max(revisions.published_at)
-- across everything live for the artist. Additive. Reproduced verbatim from
-- 20260826120000 with ONE key added.
...
as $$
  ...
  select case
    ...
    when not exists (select 1 from ap) then null
      ...
      'published_at', (select max(published_at) from live),
      'artist',
        (select data from ap)
