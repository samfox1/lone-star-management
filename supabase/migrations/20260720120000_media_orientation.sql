-- Orientation for gallery photos, so the public site can lay each one out as horizontal
-- or vertical instead of guessing. skeen's photo collage sizes each photo by aspect
-- ratio but only knows the BUNDLED images' shapes — every backend-supplied photo fell
-- back to landscape, so uploaded portraits rendered wrong. That's why the gallery was
-- effectively stuck on the hardcoded images.
--
-- The manager fills orientation-typed slots in the editor's Images panel (3 horizontal +
-- 3 vertical); the upload measures the file's real dimensions to warn on a mismatch. Only
-- gallery_image rows carry orientation; hero_video / profile_photo leave it null.
alter table public.media
  add column if not exists orientation text
  check (orientation is null or orientation in ('horizontal', 'vertical'));

-- get_public_site's media branch CHERRY-PICKS fields into a fresh object (unlike the
-- pass-through branches that emit `data` wholesale), so `orientation` has to be added
-- here or it never reaches the site. Everything else is VERBATIM from 20260714160000.
