 ...
 * currently live (`visible`), and re-seeds when the server's live set changes — after
 * a successful publish + refresh — by adjusting state during render, keyed on which
 * ids are actually visible (the sanctioned "reset state on prop change" pattern).
 *
 ...
 */
export function useOnSiteSelection<T extends { id: string; visible: boolean }>(items: T[]) {
  const liveKey = useMemo(
    () => items.filter((i) => i.visible).map((i) => i.id).sort().join(','),
    [items],
  )
  const seed = () => ({ key: liveKey, ids: new Set(items.filter((i) => i.visible).map((i) => i.id)) })
  ...
  const pendingCount = items.filter((i) => sel.ids.has(i.id) !== i.visible).length
